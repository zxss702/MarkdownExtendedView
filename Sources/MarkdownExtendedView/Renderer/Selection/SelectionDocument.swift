...
//
//  A document model for text selection: ordered sections, lines and
//  per-glyph slices with rects, plus hit-testing and copy extraction.
//  Revived from the pre-flatten architecture; adds `linePrefix` support
//  for blockquote copy (`"> "` per line).
...
struct SelectionSlice: @unchecked Sendable {
    ...
    /// Resolved absolute link destination — drives pointing-hand hover
    /// and the "拷贝链接" context menu.
    var link: String? = nil
...
struct SelectionDocument: @unchecked Sendable {
    ...
    func plainText(in range: SelectionRange) -> String {
        ...
        for section in sections {
            ...
            text += prefixedSectionText(substring, linePrefix: section.linePrefix)
    ...
    /// Applies a per-line prefix (`"> "` for quotes) to a section's copied
    /// substring: at the start, and after every interior newline. A
    /// trailing newline never produces a dangling prefix.
    private func prefixedSectionText(_ substring: String, linePrefix: String?) -> String {
        guard let linePrefix, !linePrefix.isEmpty else {
            return substring
        }
    ...
    /// drives pointing-hand hover and the "拷贝链接" menu.
    func link(at point: CGPoint) -> String? {
        slices.first {
            $0.link != nil && $0.rect.insetBy(dx: -2, dy: -2).contains(point)
        }?.link ?? nil
    }

    /// Rich ("含图像") copy: fully-selected slices with rich content emit
    /// their image/icon+label instead of the plain payload; everything
    /// else copies the attributed text verbatim. Section line breaks and
    /// blockquote prefixes mirror `plainText(in:)`.
    func richText(in range: SelectionRange) -> NSAttributedString {
        let characterRange = selectedCharacterRange(for: range)
        guard characterRange.lowerBound < characterRange.upperBound else {
            return NSAttributedString()
        }
        ...
        for section in sections {
            ...
            let sectionRich = richSectionText(
                in: lowerBound..<upperBound,
                linePrefix: section.linePrefix
            )
            guard sectionRich.length > 0 else {
                continue
            }
            ...
            if
                ...
                shouldInsertLineBreak(
                    between: previousSection,
                    and: section,
                    existingText: result.string,
                    upcomingText: sectionRich.string
                )
            {
                result.append(NSAttributedString(string: "\n"))
            }

            result.append(sectionRich)
            previousSection = section
        }
    ...
    private func richSectionText(
        ...
        for slice in slices where slice.range.intersects(range) {
            let sliceStart = max(slice.range.lowerBound, range.lowerBound)
            let sliceEnd = min(slice.range.upperBound, range.upperBound)

            if sliceStart > cursor {
                appendPlainText(cursor..<sliceStart, into: result)
            }

            if
                let rich = slice.rich,
                slice.range.lowerBound >= range.lowerBound,
                slice.range.upperBound <= range.upperBound
            {
                appendRich(rich, into: result)
            } else if sliceStart < sliceEnd {
                appendPlainText(sliceStart..<sliceEnd, into: result)
            }

            cursor = max(cursor, slice.range.upperBound)
        }

        if cursor < range.upperBound {
            appendPlainText(cursor..<range.upperBound, into: result)
        }

        return prefixedRichText(result, linePrefix: linePrefix)
    }

    private func appendPlainText(_ range: Range<Int>, into result: NSMutableAttributedString) {
        guard range.lowerBound < range.upperBound, range.upperBound <= attributedString.length else {
            return
        }
    ...
    private func appendRich(_ rich: SelectionRichContent, into result: NSMutableAttributedString) {
        switch rich {
    ...
    private static func imageAttachment(_ image: MTImage) -> NSTextAttachment {
    ...
    /// Same prefixing as `prefixedSectionText` but preserving attributes:
    /// `prefix` at the start and after every interior newline.
    private func prefixedRichText(
        _ rich: NSAttributedString,
        linePrefix: String?
    ) -> NSAttributedString {
        guard let linePrefix, !linePrefix.isEmpty else {
            return rich
        ...
        let result = NSMutableAttributedString(string: linePrefix)
        let string = rich.string as NSString
        for index in 0..<rich.length {
            result.append(rich.attributedSubstring(from: NSRange(location: index, length: 1)))
            if string.character(at: index) == 0x0A, index + 1 < rich.length {
                result.append(NSAttributedString(string: linePrefix))
            }
        }
        return result
    }
